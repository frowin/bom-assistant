      ...
      await Excel.run(async (context) => {
        ...
        const components = data
          ...
          .map((row) => ({
            ...
            qtyPerBoard: parseFloat(getCellValue(row, headers, ["qty per board", "qty"])) || 0,
            qtyPerOrder: parseFloat(getCellValue(row, headers, ["qty per order"])) || 0,
            qtyToConsign: parseFloat(getCellValue(row, headers, ["qty to consign"])) || 0,
            source: getCellValue(row, headers, ["source"]),
            provider: getCellValue(row, headers, ["provider"]),
            ordered: getCellValue(row, headers, ["ordered"]),
            minimumQty: parseFloat(getCellValue(row, headers, ["minimum qty", "min qty"])) || 0,
            bufferQty: parseFloat(getCellValue(row, headers, ["buffer qty"])) || 0,
            model: getCellValue(row, headers, ["model"]) || "",
            manufacturerPartNumber: getCellValue(row, headers, ["mfr #", "manufacturer part number"]),
